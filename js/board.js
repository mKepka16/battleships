const initBoard = (board) => {
  for (let i = 0; i < BOARD_SIZE + 2; i++) {
    ...
    for (let j = 0; j < BOARD_SIZE + 2; j++) {
      const condition =
        j == 0 || j == BOARD_SIZE + 1 || i == 0 || i == BOARD_SIZE + 1;
...
const placeRandomWarships = (board) => {
  WARSHIPS.forEach((warship) => {
    ...
    for (
      let x = goodFields[0][0] - 1;
      x <= goodFields[goodFields.length - 1][0] + 1;
      x++
    ) {
      for (
        let y = goodFields[0][1] - 1;
        y <= goodFields[goodFields.length - 1][1] + 1;
        y++
      ) {
...
const renderVirtualBoard = (board, name) => {
  ...
  boardTitleDOM.textContent = name == 'playerBoard' ? 'Player' : 'Computer';
...
const findField = (x, y, boardName = 'playerBoard') =>
  document.querySelector(`.${boardName} [data-x="${x}"][data-y="${y}"]`);
const getFieldCords = (field) => [
  field.getAttribute('data-x'),
  field.getAttribute('data-y'),
];
